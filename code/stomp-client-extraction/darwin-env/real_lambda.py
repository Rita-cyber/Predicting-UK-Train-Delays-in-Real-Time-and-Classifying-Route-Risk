...
key = 's3://darwin-raildata-mlops/darwin-raildata-mlops/raw/2025/07/22/'
...
def lambda_handler(event, context):
    ...
    darwin_csv_data = df.to_csv(index=False)

    filename = f"{PROCESSED_PREFIX}/events_snapshot_{datetime.utcnow().strftime('%Y-%m-%dT%H-%M-%S')}.csv"
    s3.put_object(Bucket=BUCKET_NAME, Key=filename, Body=darwin_csv_data)
